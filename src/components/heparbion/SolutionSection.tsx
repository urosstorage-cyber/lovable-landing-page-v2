      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8">
        ...
        >
          ...
          <h2 className="font-serif text-3xl md:text-4xl lg:text-5xl font-semibold text-white tracking-elegant mb-5">
            ...
            <span className="text-shimmer">Bitters</span>
          </h2>
          <p className="text-base text-white/40 max-w-2xl mx-auto leading-relaxed">
            Unlike generic supplements, Heparbion Plus focuses on the "Cooling & Clearing" principle — 
            a time-tested Ayurvedic approach to hepatic balance.
          </p>
          ...
          <div className={`scroll-reveal-left ${contentRevealed ? 'revealed' : ''}`}>
            <div className="space-y-8">
              {[
                {
                  icon: Droplets,
                  title: 'Cooling & Clearing',
                  desc: 'Our bitter formula targets the liver\'s natural cooling pathways, reducing inflammatory heat and supporting bile flow for optimal digestion.',
                },
                {
                  icon: Wind,
                  title: 'Vagus Nerve Stimulation',
                  desc: 'Bitter compounds activate the vagus nerve, signaling your body to shift from "fight or flight" stress mode into "rest and digest" recovery mode.',
                },
                {
                  icon: Sparkles,
                  title: 'Synergistic Botanical Blend',
                  desc: 'Each ingredient is selected not just for individual potency, but for how they amplify each other\'s therapeutic effects.',
                },
              ].map((item, i) => (
                <div
                  key={i}
          ...
          <div className={`flex justify-center scroll-reveal-right ${contentRevealed ? 'revealed' : ''}`}>
            <div className="relative">
              ...
              <div className="absolute -top-4 -right-4 glass-dark rounded-xl px-4 py-2.5 animate-float" style={{ animationDelay: '1s' }}>
                <p className="text-[10px] text-gold-400/60 uppercase tracking-wide">120 Vegan</p>
                <p className="text-sm font-medium text-white">Capsules</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div ref={ingredientRef}>
          <h3
            className={`font-serif text-2xl font-medium text-white text-center mb-10 scroll-reveal ${
              ingredientRevealed ? 'revealed' : ''
            }`}
          >
            Key Ingredients
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {ingredients.map((ing, i) => (
              <div
                key={i}
                className={`group relative rounded-2xl bg-white/[0.04] border border-white/[0.06] p-6 hover:bg-white/[0.08] hover:border-gold-400/20 transition-all duration-500 bento-item scroll-reveal ${
                  ingredientRevealed ? 'revealed' : ''
                }`}
