...
const PainPointsGrid: React.FC = () => {
  ...
  const painPoints = [
    { icon: Scale, question: t('pain.q1'), detail: t('pain.d1'), accent: 'bg-emerald-500/[0.06]', span: 'md:col-span-2 md:row-span-1' },
    { icon: CloudFog, question: t('pain.q2'), detail: t('pain.d2'), accent: 'bg-gold-400/[0.06]', span: 'md:col-span-1 md:row-span-2' },
    { icon: Zap, question: t('pain.q3'), detail: t('pain.d3'), accent: 'bg-emerald-500/[0.04]', span: 'md:col-span-1 md:row-span-1' },
    { icon: Brain, question: t('pain.q4'), detail: t('pain.d4'), accent: 'bg-gold-400/[0.04]', span: 'md:col-span-1 md:row-span-1' },
    { icon: Moon, question: t('pain.q5'), detail: t('pain.d5'), accent: 'bg-emerald-500/[0.05]', span: 'md:col-span-2 md:row-span-1' },
  ];
      ...
      <div className="relative max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-5">
          {painPoints.map((point, i) => (
            <div
              key={i}
              className={`${point.span} glass-card rounded-2xl p-6 md:p-8 bento-item scroll-reveal ${gridRevealed ? 'revealed' : ''}`}
            ...
            >
              ...
              <p className="text-sm text-emerald-500/50 leading-relaxed">{point.detail}</p>
            </div>
          ))}
        </div>

        <div className={`text-center mt-12 scroll-reveal ${gridRevealed ? 'revealed' : ''}`} style={{ transitionDelay: '600ms' }}>
          <p className="text-sm text-emerald-500/50 mb-4">{t('pain.bottom')}</p>
          <button
            onClick={() => {
