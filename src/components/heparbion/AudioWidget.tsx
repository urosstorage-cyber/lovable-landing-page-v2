    ...
    >
      {isExpanded ? (
        ...
        <div className="glass-card rounded-2xl p-5 w-72 shadow-xl animate-reveal-up">
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-xs font-medium text-emerald-500">Podcast</p>
                <p className="text-[10px] text-emerald-500/40">Aleksandra Komasz</p>
              </div>
            </div>
            <button
          ...
          {/* Title */}
          <p className="text-sm font-serif font-medium text-emerald-500 mb-1">
            Ayurvedic Liver Deep-Dive
          </p>
          <p className="text-[10px] text-emerald-500/40 mb-4">
            Understanding the bitter principle in liver health
          </p>
            ...
            {Array.from({ length: 20 }).map((_, i) => (
              <div
                ...
                className={`w-[3px] rounded-full transition-all duration-300 ${
                  isPlaying ? 'bg-gold-400' : 'bg-emerald-500/15'
                }`}
                style={{
                  height: isPlaying
                    ? `${Math.max(4, Math.random() * 32)}px`
                    : `${Math.max(4, Math.sin(i * 0.5) * 12 + 12)}px`,
                  animationDelay: `${i * 50}ms`,
                  transition: isPlaying ? 'height 0.3s ease' : 'height 0.5s ease',
                }}
              />
            ))}
          </div>
          ...
          <div className="mb-3">
            ...
            <div className="flex justify-between mt-1">
              <span className="text-[9px] text-emerald-500/30">
                {Math.floor(progress * 0.18)}:{String(Math.floor((progress * 0.18 * 60) % 60)).padStart(2, '0')}
              </span>
          ...
          <div className="flex items-center justify-center">
            <button
              onClick={togglePlay}
              className="w-10 h-10 rounded-full bg-emerald-500 text-white flex items-center justify-center hover:bg-emerald-600 transition-colors shadow-lg shadow-emerald-500/20"
            >
        ...
        >
          ...
          <div className="pr-2">
            <p className="text-xs font-medium text-emerald-500">Listen</p>
            <div className="flex items-end gap-[2px] h-3 mt-0.5">
              {Array.from({ length: 7 }).map((_, i) => (
                <div
                  ...
                  className="wave-bar"
                  style={{
                    height: `${Math.max(3, Math.sin(i * 0.8) * 8 + 8)}px`,
